...
export class New01761817281408 implements MigrationInterface {
    name = 'New01761817281408'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`teacher_subjects\` (\`id\` varchar(36) NOT NULL, \`teacherId\` varchar(255) NOT NULL, \`subjectId\` varchar(255) NOT NULL, \`fee\` decimal(10,2) NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`isDeleted\` tinyint NOT NULL DEFAULT 0, UNIQUE INDEX \`IDX_8e762322ca7fbbe06ab8eb1ea6\` (\`teacherId\`, \`subjectId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`userdetails\` ADD \`skills\` text NULL`);
        ...
        await queryRunner.query(`ALTER TABLE \`teacher_subjects\` ADD CONSTRAINT \`FK_2013034e3c170743cbd5fda6de9\` FOREIGN KEY (\`teacherId\`) REFERENCES \`users\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`classes\` DROP COLUMN \`title\``);
        await queryRunner.query(`ALTER TABLE \`classes\` DROP COLUMN \`name\``);
        await queryRunner.query(`ALTER TABLE \`userdetails\` DROP COLUMN \`skills\``);
        await queryRunner.query(`ALTER TABLE \`classes\` ADD \`name\` varchar(255) NOT NULL`);
        await queryRunner.query(`ALTER TABLE \`classes\` ADD \`type\` enum ('individual', 'academy') NOT NULL`);
        await queryRunner.query(`ALTER TABLE \`classes\` ADD \`title\` varchar(200) NOT NULL`);
        await queryRunner.query(`ALTER TABLE \`classes\` ADD \`teacherId\` varchar(255) NOT NULL`);
